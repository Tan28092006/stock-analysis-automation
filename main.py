...
# ==== Bước 1: Lọc top 5 tăng trưởng 14 ngày ====
...
for ticker in vn30_tickers:
    ...
    if df_temp.empty:
        continue
    growth = (df_temp['Close'].iloc[-1] - df_temp['Close'].iloc[0]) / df_temp['Close'].iloc[0] * 100
    growth_data.append((ticker, float(growth)))

growth_data.sort(key=lambda x: x[1], reverse=True)
top_5 = growth_data[:5]

print("📈 Top 5 cổ phiếu tăng trưởng 14 ngày:")
for t, g in top_5:
    print(f"{t}: {g:.2f}%")

# ==== Bước 2: Phân tích 28 ngày cho top 5 ====
save_path = "outputs"
...
def analyze_data_with_groq(json_data):
    prompt = (
        ...
        "và chỉ sử dụng đoạn văn bản (không dùng bảng) khi trả lời.\n\n"
        f"Dữ liệu: {json_data}\n\n"
        "Phân tích chi tiết:"
    )
...
# ==== Gom kết quả phân tích ====
json_files = glob.glob(os.path.join(save_path, "*.json"))
report_text = "Báo cáo phân tích kỹ thuật Top 5 cổ phiếu VN30:\n\n"
...
for file_path in json_files:
    ...
    try:
        ...
        print(f"❌ Lỗi API khi phân tích {ticker_name}: {e}")
...
# ==== Xuất file Word ====
doc = Document()
doc.add_heading("Báo cáo Phân tích Chỉ báo Kỹ thuật - Top 5 VN30", level=1)
...
try:
    send_email_report(
        ...
        subject="Báo cáo phân tích Top 5 cổ phiếu VN30",
        content=report_text,
